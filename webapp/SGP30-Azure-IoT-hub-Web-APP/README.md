...
This repo contains code for a web application, which can read Wio Terminal Light sensor data from IoT Hub and show the real-time data in a line chart on the web page.
...
This tutorial, also published [here](https://wiki.seeedstudio.com/Get-Wio-Terminal-data-via-Helium-Azure-oT-Hub/), shows how to set up a nodejs website to visualize device data streaming to an [Azure IoT Hub](https://azure.microsoft.com/en-us/services/iot-hub) using the [event hub SDK](https://www.npmjs.com/package/@azure/event-hubs). In this tutorial, you learn how to:
...
1. Push the repo's code to the git repo URL in last step with:
    - In the **Overview** page, find the **Git clone URL**, using the **App Service build service** build provider. Then run the following commands:
        ...
        ```cmd
        git clone https://github.com/limengdu/Wio-Terminal-light-sensor-Azure-IoT-hub-Web-APP.git
        cd Wio-Terminal-light-sensor-Azure-IoT-hub-Web-APP
